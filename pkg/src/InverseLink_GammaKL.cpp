...
void InverseLink_GammaKL::UpdatePatternMatrix()
{
...
void InverseLink_GammaKL::UpdateAmplitudeMatrix()
{
...
int InverseLink_GammaKL::CheckConvergency(int round)
{
	...
	{
		errors = myData.InverseLink_GammaKL_Div( mock->data );
		consecutiveError = fabs(errors - myData.InverseLink_GammaKL_Div( prevMock ));
		totalSteps = round;
